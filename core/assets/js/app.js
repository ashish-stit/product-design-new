...
jQuery(document).ready(function($) {
	...
	window.lumise = {
			...
			events : function(){
				...
				lumise.trigger({
					...
					events: {
						...
						'#lumise-upload-form-video input[type="file"]:change': function(){
							lumise.fn.process_video_files(this.files);
						},
					...
					leftNav: function(e) {
						...
						if (tab == 'drawing') {
							...
							if (lumise.get.el('external-images').hasClass('active')) {
								$('#lumise-external-video iframe').each(function(){
									this.contentWindow.postMessage({
										action : 'refresh'
									}, "*");
								});
                ...
                $('#lumise-upload-form-video').on('drag dragstart dragend dragover dragenter dragleave drop', function(e) {
					e.preventDefault();
					e.stopPropagation();
				})
				...
				.on('drop', function(e) {
					lumise.fn.process_video_files(e.originalEvent.dataTransfer.files);
				})
				.on('click', function(){
		...
		fn : {
			...
			process_video_files: function(files, callback, saveas) {
				var tmpl = '', file, reader  = {};
				...
				for (f in files) {
					...
					if (files[f].type.indexOf('video/') !== 0)
						return lumise.fn.notice(lumise.i('214'), 'error', 5000);
					...
					reader[f].addEventListener("load", function () {
						...
						if (!lumise.fn.check_upload_video_size(reader[this.f].file)) {
							delete reader[this.f];
							return;
						};
						...
						if (url_data.indexOf('data:video/mp4+xml;base64,') === 0) {
							var wrp = $('<div>'+atob(url_data.split('base64,')[1]).replace('viewbox=', 'viewBox=')+'</div>'),

								mp4 = wrp.find('mp4').get(0),
								vb = mp4.getAttribute('viewBox') ? 
									 mp4.getAttribute('viewBox') : 
									 mp4.getAttribute('viewbox');
								...
								if (!mp4.getAttribute('width'))
									mp4.setAttribute('width', vb[2]);
									
								if (!mp4.getAttribute('height'))
									mp4.setAttribute('height', vb[3]);
								
							wrp.find('[id]').each(function(){
								this.id = this.id.replace(/[\u{0080}-\u{FFFF}]/gu,"");
							});
							
							img_opt.url = 'data:video/mp4+xml;base64,'+btoa(wrp.html());
							
							if (saveas !== false)
								new lumise.videoparts.import(id, img_opt, 'prepend');
			...
			select_video: function(callback, saveas) {
				...
				if (ops.image_inp === undefined) {
					ops.image_inp = document.createElement('input');
					ops.image_inp.type = 'file';
					ops.image_inp.accept = '.mp4';
					ops.image_inp.onchange = function(){
						lumise.fn.process_video_files(this.files, this.callback, this.saveas);
					};
				};
			...
			check_upload_size : function(file) {
				...
				check_upload_video_size : function(file) {
				...
				lumise.get.el('stage-nav').find('li[data-stage]').each(function(i){
					...
					if (stage.canvas) {
						...
						if(activeStageStatus == true && save != 'cart'){
							...
							//alert(data.stages[s].screenshot);
						}
				...
				if (
		    		...
		    		if (
						...
						try {
							...
							lumise.indexed.save([thumbn, data], 'designs', function(e){
								delete data;
								delete thumbn;
		...
		cart : {
            ...
            */
			...
			init				: function () {
				...
				*/
				...
				$('#lumise-cart-action').on('click', function(e){
					//alert(lumise.cart.add_cart);
					
					lumise.cart.add_cart('button add cart click');
					
				$('#lumise-general-status a[href="#lumise-cart-action"]').on('click', lumise.cart.do_checkout);
				
					e.preventDefault();
				});
				$('#lumise-cart-action-edit').on('click', function(e){
					lumise.cart.update_cart('button update cart click');
					
				$('#lumise-general-status a[href="#lumise-cart-action-edit"]').on('click', lumise.cart.do_checkout);
								...
								lumise.fn.download_design({
									...
									callback: function(data) {
										...
										if (Object.keys(lumise.data.stages)[start_render] !== undefined) {
											...
											lumise.active_stage(current_stage);
											//alert('hello');
											return lumise.cart.process_update_cart(cart_design);
										}
			...
			process_update_cart	: function(cart_design) {
				lumise.f(false);
				var values			= [],
				...
				cart_data[id] = {
				    ...
				    screenshot	: '',
					stages		: 0,
					order_print :lumise.fn.url_var('order_print'),
					action:          "update",
					name		: lumise.ops.product_data.name,
				...
				alert(cart_data[id].order_print);
				Object.keys(lumise.data.stages).map(function(s) {
					cart_data[id].stages++;
    ...
    var btn = document.getElementById('fill-blanck');
    btn.addEventListener('click', function() {
      document.location.href = 'fill.php';
    });
    var btn = document.getElementById('drag-drop');
    btn.addEventListener('click', function() {
      document.location.href = 'drag-drop.php';
    });
    var btn = document.getElementById('true-false');
    btn.addEventListener('click', function() {
      document.location.href = 'true-false.php';
    });
    var btn = document.getElementById('flip-book');
    btn.addEventListener('click', function() {
      document.location.href = 'flip-book.php';
    });
    var btn = document.getElementById('games');
    btn.addEventListener('click', function() {
      document.location.href = 'games.php';
    });
