<?php
	...
	if (isset($_POST['action_submit']) && !empty($_POST['action_submit'])) {
		...
		$lumise_admin->check_caps('images');
		
		foreach ($val as $value) {

			$dt = $lumise_admin->get_row_id($value, 'images');
			switch ($data_action) {
				...
				case 'active':
					...
					$dt = $lumise_admin->edit_row( $value, $data, 'images' );
					break;
				case 'deactive':
					$data = array(
						'active' => 0
				...
				case 'delete':
					...
					$lumise_admin->delete_row($value, 'images');
	...
	if (isset($_POST['do']) && !empty($_POST['do'])) {
		$lumise->redirect($lumise->cfg->admin_url . "lumise-page=images");
		exit;
	}
    ...
    $select = "SELECT SQL_CALC_FOUND_ROWS art.* FROM {$lumise->db->prefix}images art ";
	...
	$config = array(
		...
		'total_page'    => ceil($total/$per_page),
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'lumise-page=images&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'lumise-page=images',
	);
...
<div class="lumise_wrapper">
	...
	<div class="lumise_content">
		...
		<div class="lumise_header">
			<h2><?php echo $lumise->lang('images'); ?></h2>
			<a href="<?php echo $lumise->cfg->admin_url;?>lumise-page=image" class="add-new lumise-button">
				<i class="fa fa-plus"></i>
				<?php echo $lumise->lang('Add new Image'); ?>
			</a>
			<?php if ($can_upload) { ?>
			<a href="<?php echo $lumise->cfg->admin_url;?>lumise-page=image" class="add-new lumise-button" id="lumise-add-bundle-images">
				<i class="fa fa-th"></i>
				<?php echo $lumise->lang('Add multiple Images'); ?>
			</a>
			<?php } ?>
			<?php
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise->cfg->admin_url;?>lumise-page=images" method="post">
				...
				<form action="<?php echo $lumise->cfg->admin_url;?>lumise-page=images" method="post" class="less">
						...
						foreach ($arts as $art) { ?>
							...
							<tr>
								...
								<td class="lumise-resource-title">
									<a href="<?php echo $lumise->cfg->admin_url;?>lumise-page=image&id=<?php if(isset($art['id'])) echo $art['id'] ?>" class="name"><?php if(isset($art['name'])) echo $art['name']; ?></a>
									<span> - #<?php if(isset($art['id'])) echo $art['id'] ?></span>
								</td>
								<td>
									<?php
										$art['id'] = isset($art['id']) ? $art['id'] : '';
										$dt = $lumise_admin->get_category_item($art['id'], 'images');
										$dt_name = array();
								...
								<td style="width:20%; position:relative;">
									<?php
										$art['id'] = isset($art['id']) ? $art['id'] : '';
										$dt = $lumise_admin->get_tag_item($art['id'], 'images');
										$dt_name = array();
										foreach ($dt as $val) {
											$dt_name[] = $val['name'];
										}
									?>
									<input name="tags" class="tagsfield" value="<?php echo implode(',', $dt_name); ?>" data-id="<?php echo $art['id']; ?>" data-type="images">
								</td>
								<td>
									<?php
								...
								<td>
									<a href="#" class="lumise_action" data-type="images" data-action="switch_active" data-status="<?php echo (isset($art['active']) ? $art['active'] : '0'); ?>" data-id="<?php if(isset($art['id'])) echo $art['id'] ?>">
										<?php
											if (isset($art['active'])) {
												if ($art['active'] == 1) {
		...
		<?php } else {
					if (isset($total_record[0]['total']) && $total_record[0]['total'] > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$lumise->cfg->admin_url.'lumise-page=images" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else echo '<p class="no-data">'.$lumise->lang('No data. Please add Image.').'</p>';
			}?>
...
<div id="lumise-popup">
	<div class="lumise-popup-content lumise-multi-cliparts">
		<header>
			<h3><?php echo $lumise->lang('Add bundle multiple Images'); ?></h3>
		...
		<div class="lumise-langs-wrp lumise_content">
			<div class="lumise_form_group">
				...
				<div class="lumise_form_content">
					...
					<a href="<?php echo $lumise->cfg->admin_url;?>lumise-page=categories&type=images" target=_blank class="add_cate" data-click="toggle-form">
						<i class="fa fa-plus"></i>
			...
			<div class="lumise_form_group">
				<span><?php echo $lumise->lang('Upload Images'); ?></span>
				<div class="lumise_form_group">
