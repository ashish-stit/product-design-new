...
class lumise_tmpl_register {
	...
	public function reg_editor_menus() {
		...
		return array(
				...
				"content" =>
					...
					'<header class="images-from-socials lumise_form_group">
						<button class="active" data-nav="internal">
							...
							'.$lumise->lang('Upload Video').'
						</button>
					</header>' : '').
					'<div data-tab="internal" class="active">
						<div id="lumise-upload-form-video">
							<i class="lumise-icon-cloud-upload"></i>
							<span>'.$lumise->lang('Click or drop video here').'</span>
							<input type="file" multiple="true" />
						</div>
						<div id="lumise-upload-video-list">
							<ul class="lumise-list-items"></ul>
						</div>
					</div>
					<div data-tab="external" id="lumise-external-video"></div>'
			

			),
			'music' => array(
				"label" => $lumise->lang('Music'),
				"icon" => "lumise-icon-star",
				"callback" => "",
				"load" => "music",
				"class" => "lumise-x-thumbn"
			...
			// 	"content" =>
			// 		'<header>
			// 			<span class="lumise-images-search">
			// 				<input type="search" id="lumise-images-search-inp" placeholder="'.$lumise->lang('Search images').'" />
			// 				<i class="lumisex-android-search"></i>
			// 			</span>
			// 			<div class="lumise-image-categories" data-prevent-click="true">
			// 				<button data-func="show-categories" data-type="images">
			// 					<span>'.$lumise->lang('All categories').'</span>
			// 					<i class="lumisex-ios-arrow-forward"></i>
			// 				</button>
			// 			</div>
			// 		</header>
			// 		<div id="lumise-images-list" class="smooth">
			// 			<ul class="lumise-list-items">
			// 				<i class="lumise-spinner white x3 mt2"></i>
			// 			</ul>
			// 		</div>'
			...
			'addfeature' => array(
				...
				"content" =>
					( 
					'<header class="images-from-socials lumise_form_group">
						<button id="quiz" class="active" data-nav="internal">
							'.$lumise->lang('Quiz').'
						</button>
						<button id="fill-blanck" data-nav="external1">
							'.$lumise->lang('Fill Blanks').'
						</button>
						<button id="drag-drop" data-nav="external2">
							'.$lumise->lang('Drag & Drop').'
						</button>
						<button id="true-false" data-nav="external3">
							'.$lumise->lang('True/false').'
						</button>
						<button id="flip-book" data-nav="external4">
							'.$lumise->lang('Flip Book').'
						</button>
						<button id="games" data-nav="external5">
							'.$lumise->lang('Games').'
						</button>
					</header>')
			),
