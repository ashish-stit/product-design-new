...
class lumise_lib{
	...
	*/
	...
	public function store_update_cart($order_id,$cart_data){
		...
		foreach ($cart_data['items'] as $key => $item){
			...
			*/
			...
			$update_data = array(
			 ...
			 $id = $db->where ('order_id', $order_id)->update('order_products', $update_data);
			// $sqlup = "UPDATE product_order_products SET screenshots='$design_product' WHERE order_id=$order_id";
