...
class lumise_connector {
    ...
    public function __construct() {
        ...
        $this->config = array(
            ...
            "upload_path" => 'C:\\xampp\\htdocs\\product-design\\',
    ...
    public function save_order($order_id){
		...
		$cart_data = $this->get_session('lumise_cart');
         if($order_id != '')
         {
            $store = $lumise->lib->store_update_cart($order_id,$cart_data);
         }
		 else
		 {
	...
	public function update() {
		...
		if (!empty($connector_content)) {
			...
			$connector_content = str_replace(array(
				'C:\\xampp\\htdocs\\product-design\\',
				'http://localhost/product-design/',
				'localhost',
                'root',
                '',
                'productnew',
